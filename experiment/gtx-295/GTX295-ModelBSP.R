setwd("/home/marcos/Dropbox/Doctorate/Results/2015/BSPGPU/experiment/gtx-295/")
...
PCoresNumber_GT630 <- 240;
clockFrequency_GT630 <- 1242; # Mhz  
cycleMulKepler <- 20;
cycleSumKepler <- 10;
cycleFMA <- 4

memoryBusWidth_GT630 <- 448/8; # Bytes
memoryClockRate_GT630 <- 1000; # Mhz
...
sharedMemorySize_GT630 <- 16;  # Kb
registersPerBlock_GT630 <- 16; # Kb
...
L1Effect <- 0
L2Effect <- 0
W <- c(1.1, 1, 1.1, 1, .9, .9, .8, .75)

CommGM <- ((numberthreads*N*2 - L1Effect - L2Effect + numberthreads)*latencyGlobalMemory + L1Effect*latencyL1 + L2Effect*latencyL2)/54;
...
L1Effect <- 0
L2Effect <- 0
W <- c(1, 1.5, 1.5, 1.45, 1.5, 1.2, .8, .5)

CommGM <- ((numberthreads*N*2 - L1Effect - L2Effect + numberthreads)*latencyGlobalMemory + L1Effect*latencyL1 + L2Effect*latencyL2)/28;
CommSM <- (numberthreads*N*2 + numberthreads)*latencySharedMemory/8
...
L1Effect <- 0
L2Effect <- 0
W <- c(2.5, 3.5, 4, 4, 4, 4, 4, 4)

CommGM <- ((numberthreads*N*2 - L1Effect - L2Effect + numberthreads)*latencyGlobalMemory + L1Effect*latencyL1 + L2Effect*latencyL2)/96;
...
plot(N[1:6], SpeedupMatMulGmUnSP_GT630[1:6], type="l",  log="x",axes = TRUE, xaxt="n", lty = 1, lwd=c(5,5), 
...
points(N[1:6], SpeedupMatMulGmUnSP_GT630[1:6], col = cbbPalette[1], type = "p",pch=20,cex = 2)
