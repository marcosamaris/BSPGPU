...
blocksize <- gridsizes*gridsizes
numberthreads <- threadsPerBlock * blocksize;


PCoresNumber_GT630 <- 96;
clockFrequency_GT630 <- 1620; # Mhz  
cycleMulKepler <- 20;
cycleSumKepler <- 10;
cycleFMA <- 1

memoryBusWidth_GT630 <- 128/8; # Bytes
memoryClockRate_GT630 <- 667; # Mhz
...
L2CacheSize_GT630 <- 128; # Kb
sharedMemorySize_GT630 <- 48;  # Kb
L1CacheSizeKB_GT630 <- 16; # Kb
registersPerBlock_GT630 <- 32; # Kb

### BSP MULTILEVEL MEMORY

#Cycles operations per Thread
...
L1Effect <- c(0, .575, .5, .45,.425,.3, .1,.1)*reads
L2Effect <- 0.2*reads
...
plot(N[1:7], matMulGmUnSP[1:7],type="l",log="xy", lty = 2, lwd=c(5,5), yaxt="n",xaxt="n", 
     ...
     col = "blue", ylab = "Time (milisecond)", xlab = "Size of the Matrices",  cex.lab=3,cex.main=3,
     main = paste("Matrix Multiplication (#1) - ", GPU, sep =""), axes = FALSE);
...
axis(1, at = c(N), labels = c(N), cex.axis = 2.5,lwd=c(5,5))

y <- floor(log10(range(matMulGmUnSP,timeKernel_GM_Un)))
pow <- seq(y[1], y[2]+1);
axis(2, 10^pow, cex.axis = 2.5,lwd=c(5,5));

legend('bottomright', c("Predicted time","Measured time"), 
       lty=1, col=c("red","blue"), pch=c(20,21), lwd=c(5,5), cex=3)
...
plot(N[dataN], SpeedupMatMulGmUnSP_GT630[dataN], type="l",  log="x",axes = TRUE, xaxt="n", lty = 4, lwd=c(7.5,7.5), 
     ylim=c(0.5, 1.5), xlim=c(256, 8192),
     col=cbbPalette[1], ylab = " ", main=" ", cex.axis=3.5, cex.lab=3.5, cex.main=3.5,
     xlab = " ");

points(N[dataN], SpeedupMatMulGmUnSP_GT630[dataN], col = cbbPalette[1],
pch=20,cex = 3.5)

lines(N[dataN], SpeedupMatMulGmCoSP_GT630[dataN], col =
cbbPalette[2],lwd=c(7.5,7.5), lty = 3); points(N[dataN],
SpeedupMatMulGmCoSP_GT630[dataN], col = cbbPalette[2], pch=21,cex = 3.5)

lines(N[dataN], SpeedupMatMulSmUnSP_GT630[dataN], col =
cbbPalette[3],lwd=c(7.5,7.5), lty = 2); points(N[dataN],
SpeedupMatMulSmUnSP_GT630[dataN], col = cbbPalette[3], pch=22,cex = 3.5)

lines(N[dataN], SpeedupMatMulSmCoSP_GT630[dataN], col =
cbbPalette[4],lwd=c(7.5,7.5), lty = 1); points(N[dataN],
SpeedupMatMulSmCoSP_GT630[dataN], col = cbbPalette[4], pch=23,cex = 3.5)

axis(1, at = c(N[dataN]), labels = c(N[dataN]), cex.axis = 1.5) #axis(2,
at=c(SpeedupMatMulGmUnSP_GT630), cex.axis = 1.5); grid()
